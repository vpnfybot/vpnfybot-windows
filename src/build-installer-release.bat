...
if /I "%~1"=="--package-only" goto package_only

set "VPNFY_SKIP_AUTO_INSTALLER=1"
cargo build --release %*
if errorlevel 1 exit /b %errorlevel%
set "APP_BINARY=%CD%\target\release\vpnfybot-windows.exe"
goto package_installer

:package_only
if "%~2"=="" (
  echo Release binary path is required for --package-only.
  exit /b 1
)
for %%I in ("%~2") do set "APP_BINARY=%%~fI"

:package_installer
if not exist "%APP_BINARY%" (
  echo Release binary not found: %APP_BINARY%
  exit /b 1
)
...
copy /Y "%APP_BINARY%" "%PAYLOAD_DIR%\vpnfybot-windows.exe"
if errorlevel 1 exit /b %errorlevel%
...
if not defined MAKENSIS if exist "%LOCALAPPDATA%\NSIS\makensis.exe" set "MAKENSIS=%LOCALAPPDATA%\NSIS\makensis.exe"
