...
fn get_legacy_subscription_notification_state_path() -> Option<PathBuf> {
    let mut path = get_config_storage_path()?;
    path.set_file_name("subscription_notification_state.txt");
    Some(path)
}

pub(super) fn clear_legacy_subscription_notification_state() {
    if let Some(path) = get_legacy_subscription_notification_state_path() {
